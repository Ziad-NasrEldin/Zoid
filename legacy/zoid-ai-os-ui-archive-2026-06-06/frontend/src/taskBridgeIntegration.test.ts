...
{
  const current = createInitialTaskBridgeState("tasks");
  const next = applyBridgeStateToTaskUi(current, { mode: "ready", selectedTaskId: "task-1", tasks: [baseTask] });
  assertEqual(next.form.title, baseTask.title, "auto-selected first task should hydrate controlled editor title");
  assertEqual(next.form.detail, baseTask.detail, "auto-selected first task should hydrate controlled editor detail");
  assertEqual(next.form.priority, baseTask.priority, "auto-selected first task should hydrate controlled editor priority");
  assertEqual(next.formErrors.title, undefined, "auto-selected first task should clear stale form errors");
}

{
  const current = createInitialTaskBridgeState("tasks");
  const draft = { ...current.form, title: "Draft create task" };
  const next = applyBridgeStateToTaskUi({ ...current, form: draft }, { mode: "ready", selectedTaskId: null, tasks: [] });
  assertEqual(next.form.title, "Draft create task", "create-mode refresh without selection should preserve draft form");
}

{
  ...
  assertEqual(state.selectedTaskId, "task-1", "refresh should auto-select the first persisted task when nothing is selected");
