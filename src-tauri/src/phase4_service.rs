...
fn launch_gate_evidence_from_row(
    row: &rusqlite::Row<'_>,
) -> rusqlite::Result<LaunchGateEvidenceRecord> {
...
fn validate_repo_profile_input(input: &RepoProfileInput) -> RepoResult<()> {
    ...
    .contains(&input.profile_type.as_str())
    {
        return Err(RepositoryError::Constraint {
            entity: "repo_profiles",
            message: format!("unsupported repo profile type: {}", input.profile_type),
        });
    }
    if let Some(value) = input.default_branch.as_deref() {
        phase4_validate_text_field("default_branch", value, 1, 128)?;
    }
    if let Some(value) = input.package_manager.as_deref() {
        phase4_validate_text_field("package_manager", value, 1, 64)?;
    }
    if let Some(value) = input.linked_product_id.as_deref() {
        phase4_validate_text_field("linked_product_id", value, 1, 128)?;
    }
...
#[allow(dead_code)]
pub(crate) fn add_repo_profile(
    connection: &Connection,
    input: RepoProfileInput,
) -> RepoResult<RepoProfileRecord> {
    validate_repo_profile_input(&input)?;
    let id = format!(
        "repo-{}-{}",
        now_millis(),
        REPO_PROFILE_COUNTER.fetch_add(1, Ordering::Relaxed)
    );
    connection.execute(
    ...
    read_repo_profile(connection, &id)?.ok_or_else(|| RepositoryError::NotFound {
        entity: "repo_profiles",
        key: id,
    })
}
...
#[allow(dead_code)]
pub(crate) fn read_repo_profile(
    connection: &Connection,
    repo_id: &str,
) -> RepoResult<Option<RepoProfileRecord>> {
...
#[allow(dead_code)]
pub(crate) fn link_repo_entity(
    connection: &Connection,
    input: RepoEntityLinkInput,
) -> RepoResult<EntityLinkRecord> {
    read_repo_profile(connection, &input.repo_id)?.ok_or_else(|| RepositoryError::NotFound {
        entity: "repo_profiles",
        key: input.repo_id.clone(),
    })?;
    if !["task", "product"].contains(&input.target_type.as_str()) {
        return Err(RepositoryError::Constraint {
            entity: "entity_links",
            message: "Phase 4 repo links only support task/product targets".to_string(),
        });
    }
...
#[allow(dead_code)]
pub(crate) fn list_phase4_repo_integration_states(
    connection: &Connection,
) -> RepoResult<Vec<IntegrationStatusRecord>> {
    for (key, label, note) in [
        (
            "github",
            "GitHub",
            "State only; no deep GitHub automation or git reads are implemented.",
        ),
        (
            "vercel",
            "Vercel",
            "State only; deploy execution is not implemented.",
        ),
    ] {
        let config_json =
            serde_json::json!({ "phase": "4", "scope": "state_only", "note": note }).to_string();
        connection.execute(
...
#[allow(dead_code)]
pub(crate) fn create_launch_gate(
    connection: &Connection,
    input: LaunchGateCreateInput,
) -> RepoResult<LaunchGateRecord> {
    read_repo_profile(connection, &input.repo_id)?.ok_or_else(|| RepositoryError::NotFound {
        entity: "repo_profiles",
        key: input.repo_id.clone(),
    })?;
    if let Some(value) = input.product_id.as_deref() {
        phase4_validate_text_field("product_id", value, 1, 128)?;
    }
    if let Some(value) = input.task_id.as_deref() {
        phase4_validate_text_field("task_id", value, 1, 128)?;
    }
    validate_json_field("metadata_json", &input.metadata_json)?;
    validate_no_secret_json("metadata_json", &input.metadata_json)?;
    let id = format!(
        "launch-gate-{}-{}",
        now_millis(),
        LAUNCH_GATE_COUNTER.fetch_add(1, Ordering::Relaxed)
    );
    connection.execute(
    ...
    read_launch_gate(connection, &id)?.ok_or_else(|| RepositoryError::NotFound {
        entity: "launch_gates",
        key: id,
    })
}
...
#[allow(dead_code)]
pub(crate) fn read_launch_gate(
    connection: &Connection,
    launch_gate_id: &str,
) -> RepoResult<Option<LaunchGateRecord>> {
    connection.query_row(
...
#[allow(dead_code)]
pub(crate) fn add_launch_gate_evidence(
    connection: &Connection,
    input: LaunchGateEvidenceInput,
) -> RepoResult<LaunchGateEvidenceRecord> {
    read_launch_gate(connection, &input.launch_gate_id)?.ok_or_else(|| {
        RepositoryError::NotFound {
            entity: "launch_gates",
            key: input.launch_gate_id.clone(),
        }
    })?;
    if ![
        "manual_note",
    ...
    .contains(&input.evidence_type.as_str())
    {
        return Err(RepositoryError::Constraint {
            entity: "launch_gate_evidence",
            message: format!("unsupported evidence type: {}", input.evidence_type),
        });
    }
    phase4_validate_text_field("label", &input.label, 1, 256)?;
    if let Some(value) = input.url.as_deref() {
        phase4_validate_text_field("url", value, 1, 2048)?;
    }
    if let Some(code) = input.status_code {
        if !(100..=599).contains(&code) {
            return Err(RepositoryError::Constraint {
                entity: "launch_gate_evidence",
                message: "status_code must be 100-599".to_string(),
            });
        }
    }
    if let Some(value) = input.manual_note.as_deref() {
        phase4_validate_text_field("manual_note", value, 1, 4096)?;
    }
    validate_json_field("metadata_json", &input.metadata_json)?;
    validate_no_secret_json("metadata_json", &input.metadata_json)?;
    let id = format!(
        "launch-evidence-{}-{}",
        now_millis(),
        LAUNCH_GATE_EVIDENCE_COUNTER.fetch_add(1, Ordering::Relaxed)
    );
    connection.execute(
...
#[allow(dead_code)]
pub(crate) fn evaluate_launch_gate(
    connection: &Connection,
    launch_gate_id: &str,
) -> RepoResult<LaunchGateRecord> {
    read_launch_gate(connection, launch_gate_id)?.ok_or_else(|| RepositoryError::NotFound {
        entity: "launch_gates",
        key: launch_gate_id.to_string(),
    })?;
    ...
    let (state, verdict) = if evidence_count > 0 {
        ...
        (
            LaunchGateState::VerificationBlocked,
...
pub(crate) fn normalize_launch_action_policy_category(action: &str) -> Result<String, String> {
    ...
    match normalized.as_str() {
        ...
        "deploy" | "redeploy" | "rollback" | "deploy_redeploy_rollback" => {
            Ok("deploy_redeploy_rollback".to_string())
        }
        _ => Err(format!(
            "unsupported Phase 4 protected action preview: {action}"
        )),
    }
}
